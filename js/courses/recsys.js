  ...
  lessons: [
    {
      ...
      steps: [
        { t: 'text', title: 'A giant, mostly-empty grid', md: `
          <p>Picture a colossal table: one row per user, one column per item (movie, product, song). A cell holds how much that user likes that item. The recommender\'s job: <strong>fill in the blanks</strong> — predict ratings for items a user hasn\'t seen, then show the highest predicted ones.</p>
          <p>The catch: this matrix is almost entirely empty. With a million users and a million items, any one user has interacted with a vanishing fraction. The data is <strong>massive and extremely sparse</strong> — and that shapes every technique that follows.</p>` },
        { t: 'quiz',
        ...
        { t: 'text', title: 'Explicit vs implicit signals', md: `
          <p>Where do the ratings come from? Two sources: <strong>explicit feedback</strong> — the user directly rates something, 5 stars or a thumbs-up (accurate, but rare) — and <strong>implicit feedback</strong> — behavior that reveals preference: watched to the end, clicked, added to cart, replayed, scrolled past (abundant and always-on, but noisy).</p>
        ...
        { t: 'widget', name: 'classify', title: 'Try it: explicit or implicit?', md: `
          <p>Sort each user behavior into the bucket it belongs in.</p>`,
          buckets: ['Explicit feedback', 'Implicit feedback'],
          items: [
            ['Gave the movie 5 stars', 0],
            ['Watched the video all the way to the end', 1],
            ['Left a thumbs-up on the song', 0],
            ['Added the item to their cart', 1],
            ['Scrolled past without clicking', 1],
            ['Wrote a text review rating the product', 0],
            ['Replayed the track three times in a row', 1],
          ] },
        { t: 'quiz',
    ...
    },
    {
      ...
      steps: [
        { t: 'text', title: 'People like you liked...', md: `
          <p>The breakthrough idea needs no understanding of the items at all. <strong>Collaborative filtering</strong> predicts your preferences from the behavior of <em>other</em> users who are similar to you, in two flavors:</p>
          <ul>
            <li><strong>User-based</strong> — "find users with taste like yours; recommend what they liked that you haven\'t seen."</li>
            <li><strong>Item-based</strong> — "you liked X; find items people tend to like alongside X." (Amazon\'s classic "customers who bought this also bought.")</li>
          </ul>
          <p>The magic: the system needn\'t know a movie\'s genre or a product\'s specs. It learns purely from the <em>pattern of who-likes-what</em> — the crowd\'s behavior encodes the similarities.</p>` },
        { t: 'quiz',
        ...
        { t: 'text', title: 'The cold-start problem', md: `
          <p>Collaborative filtering has an Achilles\' heel: a <strong>brand-new user</strong> with no history has no similar users to compare against, and a <strong>brand-new item</strong> nobody has touched yet can\'t be recommended by co-occurrence. This is the <strong>cold-start problem</strong>.</p>
          <p>It\'s why apps ask "pick a few things you like" at signup (bootstrapping your profile), and why new items often get a deliberate exploration boost or fall back on content-based features (genre, description) until interaction data accumulates.</p>` },
        { t: 'widget', name: 'match', title: 'Try it: match the term to its meaning', md: `
          <p>Pair each collaborative-filtering term with what it means.</p>`,
          pairs: [
            ['User-based filtering', 'Find users with taste like yours; recommend what they liked'],
            ['Item-based filtering', 'Find items people tend to like alongside ones you already liked'],
            ['Cold-start problem', 'No interaction history yet, so there\'s no co-occurrence pattern to learn from'],
            ['Content-based fallback', 'Use item features like genre or description when interaction data is missing'],
          ] },
        { t: 'quiz',
    ...
    },
    {
      ...
      steps: [
        { t: 'text', title: 'Learn a vector for everything', md: `
          <p>Modern recommenders turn collaborative filtering into the geometry from the embeddings lessons. The idea, <strong>matrix factorization</strong>: learn a vector (an embedding) for every user and every item, in the same shared space. The dimensions aren\'t labeled, but they capture latent taste factors the model discovers on its own — maybe one axis tracks "gritty ↔ lighthearted," another "mainstream ↔ niche."</p>
          <p>Predicted preference is the <strong>dot product</strong> of the two vectors: aligned user and item vectors → high predicted rating. Training adjusts all the vectors by gradient descent so dot products match the known ratings — then the unknown ones become predictions.</p>` },
        ...
        { t: 'text', title: 'Why vectors beat raw neighbor-counting', md: `
          <p>Learned embeddings fix collaborative filtering\'s biggest weakness. Raw user-similarity needs two users to have rated the <em>same</em> items to compare them; embeddings don\'t — by compressing everyone into a shared low-dimensional space, the model connects users through <em>chains</em> of overlapping taste, and comparing short vectors is far cheaper than scanning millions of raw histories.</p>
          <p>This is why deep-learning recommenders (two-tower networks, etc.) now dominate: they learn rich user and item embeddings and rank by similarity — the same pattern powering search and RAG.</p>` },
        { t: 'quiz',
    ...
    },
    {
      ...
      steps: [
        { t: 'text', title: 'From prediction to a feed', md: `
          ...
          <p>That last objective is fateful: when the system is tuned to maximize engagement, it recommends whatever keeps you scrolling — not always what\'s good for you, or even what you\'d say you prefer.</p>` },
        { t: 'widget', name: 'order', title: 'Try it: order the ranking pipeline', md: `
          <p>Before you ever see a feed, candidates pass through several stages. Click them into the order a real recommender runs them in.</p>`,
          items: [
            'Candidate generation — pull a broad pool of possibly-relevant items from the whole catalog',
            'Filtering — drop ineligible items (already seen, out of stock, blocked)',
            'Ranking — score and order what\'s left by predicted preference and other objectives',
            'Re-ranking — adjust the final order for diversity, freshness, or business rules',
            'Serve the feed — show the user the top of the ranked list',
          ] },
        { t: 'quiz',
        ...
        { t: 'text', title: 'Feedback loops and filter bubbles', md: `
          <p>Recommenders have a property most ML systems don\'t: <strong>they shape the very data they learn from.</strong> The system shows you things → you interact → those interactions become training data → it shows you more of that. A <strong>feedback loop</strong>.</p>
          <p>This produces <strong>filter bubbles</strong>: show someone one viewpoint, they engage, they get more of it, narrowing what they ever see. It amplifies biases (popular items get shown more, becoming more popular regardless of quality) and traps users in narrow niches. Combating it takes deliberate <strong>exploration</strong> and diversity injection — the same explore/exploit tension from reinforcement learning, now with social stakes.</p>` },
        { t: 'quiz',
