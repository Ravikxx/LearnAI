  ...
  lessons: [
    {
      ...
      steps: [
        { t: 'text', title: 'Models don\'t see words', md: `
          <p>A neural network eats numbers, not text. So step one of every LLM is chopping text into <strong>tokens</strong> — chunks from a fixed vocabulary (50,000–200,000 entries), each with an ID number. Tokens are usually <em>pieces</em> of words: common words get their own token, rare ones split — <code>unbelievable</code> → <code>un</code> + <code>believ</code> + <code>able</code>.</p>
          <p>Why not whole words? Infinite vocabulary (every typo, name, slang word ever). Why not single letters? Sequences get enormously long. Sub-word pieces are the sweet spot: anything can be encoded, common things stay compact.</p>` },
    ...
    },
    {
      ...
      steps: [
        { t: 'text', title: 'From IDs to meaning', md: `
          <p>Token #8180 is just an ID — meaningless on its own. Step two: each token maps to an <strong>embedding</strong>, a long list of numbers (thousands of them) acting as coordinates in a "meaning space." The rule: <strong>similar meaning = nearby points.</strong> "cat" and "kitten" sit close together; "cat" and "carburetor" are far apart.</p>
          <p>Nobody designs these coordinates — they're learned parameters, tuned by gradient descent. Words that appear in similar contexts get pushed toward each other during training.</p>` },
    ...
    },
    {
      ...
      steps: [
        { t: 'text', title: 'Words need to talk to each other', md: `
          <p>Consider: <em>"The cat sat on the mat because <strong>it</strong> was tired."</em> What does "it" refer to? You instantly know: the cat (mats don't get tired) — but only after consulting <em>other words in the sentence</em>.</p>
          <p><strong>Attention</strong> is the mechanism that lets tokens do this: each token looks at every other token, decides how <em>relevant</em> each one is, and pulls in information from the relevant ones — updating its own embedding with context. After attention, "it" isn't a generic pronoun-point anymore; its embedding has absorbed cat-ness.</p>` },
        ...
        { t: 'text', title: 'How relevance is computed', md: `
          <p>The mechanics, minus the matrix algebra — each token produces three vectors (via learned weights):</p>
          <ul>
            ...
            <li>a <strong>value</strong> — the information to hand over if selected.</li>
          </ul>
          <p>Query compares against every key; strong matches get high attention weights, and the token blends in the matching <em>values</em>, weighted by those scores. This runs with many <strong>attention heads</strong> in parallel per layer — one might track pronouns, another grammar, another topic.</p>` },
        { t: 'text', title: 'The Transformer', md: `
          <p>Stack it all: tokens → embeddings → repeat dozens of times <em>[attention layer → small neural network]</em> → predict. That stack is the <strong>Transformer</strong>, from the 2017 paper "Attention Is All You Need" — the architecture behind GPT, Claude, Gemini, and Llama. It beat RNNs (which read one word at a time) for two reasons:</p>
          <ul>
            <li><strong>Every token attends to every other directly</strong> — no long-distance decay: word #5,000 can look straight at word #1.</li>
            <li><strong>All tokens process in parallel</strong> — perfect for GPUs, scaling training to internet-sized data.</li>
          </ul>
    ...
    },
    {
      ...
      steps: [
        { t: 'text', title: 'The only trick LLMs know', md: `
          ...
          <p>"The cat sat on the ___" → ~100,000 probabilities: <code>mat 62%, floor 11%, couch 8%, ... carburetor 0.0001%</code>. One token gets picked, appended, and the loop runs again — at dozens of tokens per second, that's ChatGPT typing.</p>` },
        ...
        { t: 'text', title: 'Where "hallucination" comes from', md: `
          <p>Now the most infamous LLM failure explains itself: the model produces <em>plausible</em> next tokens — that's its entire training objective. "The capital of France is" → "Paris" is both plausible AND true. But "The 2019 Henderson v. Blake ruling established" → the model continues with plausible-sounding legal language <em>whether or not the case exists</em>.</p>
    ...
    },
    {
      ...
      steps: [
        { t: 'text', title: 'Pretraining: reading the internet', md: `
          <p><strong>Stage 1: Pretraining.</strong> Take a Transformer with billions of random weights. Feed it trillions of tokens of text. At every position: predict the next token, raise the loss on wrong predictions, backprop and adjust weights — for months, on thousands of GPUs. To get good at this game, the model is forced to absorb grammar, facts, styles, code, and a surprising amount of reasoning, because all of it helps predict what comes next.</p>
        ...
        { t: 'text', title: 'Fine-tuning and RLHF: learning to be helpful', md: `
          <p><strong>Stage 2: Instruction fine-tuning.</strong> Continue training on curated conversation examples: question → helpful answer. The model learns the <em>format and persona</em> of an assistant.</p>
          <p><strong>Stage 3: RLHF</strong> (Reinforcement Learning from Human Feedback). Humans compare pairs of model answers ("this one's better"); a separate <em>reward model</em> learns to predict those preferences, and the LLM is tuned to produce answers the reward model scores highly. Pretrain → fine-tune → RLHF is the pipeline that turned autocomplete into ChatGPT.</p>` },
        { t: 'quiz',
        ...
        { t: 'widget', name: 'order', title: 'Try it: order the training pipeline', md: `
          <p>Click these into the order they actually happen in, from raw weights to chat-ready assistant.</p>`,
          items: [
            'Pretraining — predict the next token across trillions of tokens of internet text',
            'Base model — a fluent autocompleter that isn\'t yet a helpful assistant',
            'Instruction fine-tuning — train on curated question → helpful-answer conversations',
            'RLHF — a reward model trained on human preference comparisons steers the final tuning',
            'Chat-tuned assistant — the model you actually talk to',
          ] },
